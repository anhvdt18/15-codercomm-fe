...
const slice = createSlice({
  ...
  reducers: {
    ...
    getAddingRequestsSuccess(state, action) {
      state.isLoading = false;
      state.error = null;
...
// Tuan Anh Codding Start
export const getAddingRequests =
  ({ filterName, page = 1, limit = 12 }) =>
  async (dispatch) => {
    dispatch(slice.actions.startLoading());
    try {
      ...
      dispatch(slice.actions.getAddingRequestsSuccess(response.data));
    } catch (error) {
      dispatch(slice.actions.hasError(error.message));
      toast.error(error.message);
    }
